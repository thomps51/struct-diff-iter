...
pub fn lazy_struct_diff_derive(input: TokenStream) -> TokenStream {
    let ast = syn::parse(input).unwrap();
    impl_lazy_struct_diff(&ast)
}
...
fn impl_data_struct(name: &Ident, data_struct: &DataStruct) -> TokenStream {
        ...
        Fields::Unit => {
            let gen = quote! {
                ...
                impl LazyDiff for #name {
                    fn lazy_diff_iter<'a, 'b: 'a>(&'a self, other: &'b Self) -> impl Iterator<Item = ::lazy_struct_diff::DiffData> {
                        [].into_iter()
                    }
                }
            };
    ...
    let gen = quote! {
        ...
        impl LazyDiff for #name {
            fn lazy_diff_iter<'a, 'b: 'a>(&'a self, other: &'b Self) -> impl Iterator<Item = ::lazy_struct_diff::DiffData> {
                use ::lazy_struct_diff::itertools::Itertools;
                #(#field_writes)*
            }
        }
    };
...
fn impl_enum(name: &Ident, variants: &Punctuated<Variant, Comma>) -> TokenStream {
    ...
    let gen = quote! {
        ...
        impl LazyDiff for #name {
            fn lazy_diff_iter<'a, 'b: 'a>(&'a self, other: &'b Self) -> impl Iterator<Item = ::lazy_struct_diff::DiffData> {
                use ::lazy_struct_diff::itertools::Itertools;
                let iter: Box<dyn Iterator<Item = ::lazy_struct_diff::DiffData>> = match (self, other) {
                    #(#variants)*
                    (_, _) => Box::new(
                        [::lazy_struct_diff::DiffData {
                            field: ::lazy_struct_diff::FieldIdentifier::new(),
                            self_value: self,
                            other_value: other,
                        }]
                        .into_iter(),
...
fn impl_lazy_struct_diff(ast: &syn::DeriveInput) -> TokenStream {
    let name = &ast.ident;
    match &ast.data {
